...
export const ScheduleScreen: React.FC<ScheduleScreenProps> = ({
    ...
    const handleRegisterMyShift = (dayIndex: number, shiftType: "morning" | "afternoon" | "evening") => {
        // Check if CTV is pending approval
        if (ctvUser.status === "Chờ duyệt") {
            onShowToast("Hồ sơ của bạn đang chờ duyệt. Vui lòng đợi Admin phê duyệt trước khi đăng ký ca làm việc.");
            return;
        }
    ...
    const getFilteredCTVs = (ctvs: AssignedCTV[] = []) => {
        const nonAdminCTVs = ctvs.filter((c) => {
            const userAcc = accounts.find((a) => a.id === c.id || a.name.toLowerCase() === c.name.toLowerCase());
                                                    ...
                                                    if (onViewAccountDetail) {
                                                        ...
                                                        if (matched) {
                                                            ...
                                                            onViewAccountDetail({
                                                                ...
                                                                initials:
                                                                    ctv.initials || ctv.name.slice(0, 2).toUpperCase(),
                                                                avatar: ctv.avatar,
