...
export const ViewRequestModal: React.FC<ViewRequestModalProps> = ({ request, onClose, onApprove, onReject }) => {
    ...
    const fallbackName = `${(request.cvFileName || `CV_${request.name}`)
        .replace(/\.[^.]+$/, "")
        .replace(/\s+/g, "_")}.txt`;
    const downloadHref = request.cvFile || `data:text/plain;charset=utf-8,${encodeURIComponent(fallbackContent)}`;
