    ...
    return (
        <div className="relative">
            <button
                ...
                <span
                    className={`material-symbols-outlined text-[18px] ml-1 ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>
                    expand_more
                </span>
            </button>
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
    const { isDarkMode, contrast, accentColor, toggleDarkMode, setContrast, setAccentColor } = useSystemSettings();
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
            <div
                className={`rounded-2xl border shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-150 ${
                    isDarkMode ? "bg-[#1e1e20] text-white border-slate-800" : "bg-white text-slate-800 border-slate-200"
                }`}>
                {/* Header */}
                <div
                ...
                {/* Content list */}
                <div className={`p-2 sm:p-4 divide-y ${isDarkMode ? "divide-slate-800/60" : "divide-slate-100"}`}>
                    {/* Row 1: Giao diện */}
                    <div
                ...
                <div
                    className={`p-4 border-t flex items-center justify-end ${
                        isDarkMode ? "bg-[#18181a] border-slate-800/80" : "bg-slate-50/80 border-slate-200"
                    }`}>
                    <button
                        onClick={onClose}
